...
本项目是 **教育心理方向网站** 的 **网页前端**（用户能在浏览器里看到和点击的那一层）。当前仓库已完成 **Week1 + Week2** 迭代：支持浏览**首页**、**心理测评**（列表、详情、模拟作答）、**课程**（列表、详情）、**用户中心（我的）**，并提供 **登录/注册 mock 页面**。文案为中文，数据以 **演示占位内容** 为主。

**不在本仓库里、以后会由其他人提供的部分**：真正的服务器、用户账号与登录、测评题目与结果保存、付费等。这些接好后，本网站会改成从服务器取数据；现在本地运行即可看到页面结构与流程。

更细的产品说明写在 `docs/week1/` 与 `docs/week2/` 下：  
- `week1`：信息结构与流程基线  
